...
const want=/(^|[\s,>+~(:])(article|\.page-wrap|main|\.article-main)(?![\w-])/;
for(const f of files){
  let root;try{root=postcss.parse(fs.readFileSync(f,'utf8'),{from:f});}catch(e){continue;}
  root.walkRules(r=>{
    const sel=(r.selector||'').replace(/\s+/g,' ');
    if(!want.test(sel))return;
    if(/hrail|reader-root|home|nagornaya|izbrannoe|series|bio-|h-|genesis|manuscript|samizdat/.test(sel))return;
    ...
    console.log(`[${f.replace('dist/','')}] [${chain(r)}] ${sel.slice(0,150)}`);

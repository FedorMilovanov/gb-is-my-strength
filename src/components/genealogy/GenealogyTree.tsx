...
// ─────────────────────────────────────────────────────────────────────────────
//  Layout: dagre, generations on Y by AM-year, siblings spread on X
// ─────────────────────────────────────────────────────────────────────────────
const NODE_W = 172;
const NODE_H = 72;
...
function buildLayout(
  persons: any[],
  opts: { showGolden: boolean; showLineage: string },
): { nodes: Node[]; edges: Edge[]; goldenPath: Set<string> } {
  const filtered = opts.showLineage === 'all'
    ? persons
    : opts.showLineage === 'messianic'
      ? persons.filter(p => p.lineage.startsWith('messianic'))
      : persons.filter(p => p.lineage === opts.showLineage);

  const ids = new Set(filtered.map(p => p.id));

  // Determine golden (messianic) path: trace from Christ back to Adam via father
  ...
  if (christ) {
    ...
    while (cur && !guard.has(cur.id)) {
      goldenPath.add(cur.id);
      guard.add(cur.id);
      cur = cur.father ? persons.find(p => p.id === cur.father) : null;
      // For Mary, her father is heli_lk; for Jesus, mother is mary
      if (cur?.id === 'jesus' && cur.mother) cur = persons.find(p => p.id === cur.mother);
    }
  }

  // Y position: by birthAM (chronological), fallback to graph depth
  ...
  const Y_SPAN = 4200; // px for the whole timeline
  ...
  filtered.forEach(p => {
    const parent = p.father && ids.has(p.father) ? p.father : (p.mother && ids.has(p.mother) ? p.mother : null);
  ...
  const nodes: Node[] = filtered.map(p => {
    ...
    const isGolden = opts.showGolden && goldenPath.has(p.id);
    return {
      id: p.id,
      type: 'person',
...
// ─────────────────────────────────────────────────────────────────────────────
//  Person Node — premium parchment card with life-bar
// ─────────────────────────────────────────────────────────────────────────────
const PersonNode = ({ data }: { data: any }) => {
  ...
  // Life bar width: relative to max biblical lifespan (969 = Methuselah)
  ...
  return (
    <div
      ...
      style={{
        ...
        border: `1.5px solid ${data.golden ? '#ffd700' : ls.border}`,
        borderRadius: '10px',
    ...
    >
      ...
      {data.disputed && (
        ...
        }} title="Спорное место — нажмите для подробностей">?</div>
      )}
      {/* name */}
      <div style={{
      ...
      {isMessiah && (
        <div style={{
          ...
          pointerEvents: 'none', animation: 'pulse-gold 2.5s ease-in-out infinite',
        }} />
      )}
    </div>
  );
...
// ─────────────────────────────────────────────────────────────────────────────
//  Detail Panel — slide-in sidebar with full person info
// ─────────────────────────────────────────────────────────────────────────────
function DetailPanel({ person, onClose }: { person: any; onClose: () => void }) {
  ...
  const roleLabels: Record<string,string> = {
    patriarch: 'Патриарх', matriarch: 'Праматерь', king: 'Царь', prince: 'Князь',
    priest: 'Священник', prophet: 'Пророк', governor: 'Правитель',
    messiah: 'Мессия', 'foster-father': 'Приёмный отец', person: 'Личность',
  };
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', marginTop: '10px' }}>
        ...
        }}>{roleLabels[person.role] || person.role}</span>
        {person.gender === 'f' && (
          <span style={{ fontSize: '10px', padding: '3px 9px', borderRadius: '999px', background: 'rgba(200,100,140,0.1)', color: '#d4889a', border: '1px solid rgba(200,100,140,0.2)' }}>жена/мать</span>
        )}
      </div>
...
export default function GenealogyTree({ persons, eras }: { persons: any[]; eras?: any[] }) {
  ...
  useEffect(() => {
    ...
    const match = persons.find(p =>
      ...
      (p.name.altName && p.name.altName.toLowerCase().includes(q)) ||
  ...
  return (
    <div style={{
      ...
      {/* Title bar */}
      <div style={{
        position: 'absolute', top: '10px', left: '14px', zIndex: 11,
      ...
      <div style={{
        ...
        <input
          type="text" placeholder="🔍 Поиск имени..." value={search}
        ...
        {lineages.map(l => (
          <button key={l.id} onClick={() => setShowLineage(l.id)} style={{
        ...
        <span style={{ color: 'rgba(200,184,154,0.3)', fontSize: '11px' }}>|</span>
        <button onClick={() => setShowGolden(g => !g)} title="Золотая мессианская нить" style={{
      ...
      >
        ...
        <MiniMap
          ...
          nodeColor={(n: any) => getLine((n.data as any)?.lineage || 'neutral').fill}
          nodeStrokeWidth={3}

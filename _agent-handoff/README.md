...
>
> **Порядок чтения: `/AGENTS.md` (договор) → этот файл → `ROADMAP.md` (план фазы 2 — РЕШЕНИЯ владельца) → `PATTERN.md`.**

| Поле | Значение |
|---|---|
| Обновлено | 2026-06-11 (ночь) |
| Кто обновил | Arena Agent (session: продолжение после упавшего агента) |
| Текущая фаза | Фаза 1 (каркас) ✅ завершена. **Фаза 2 🚧 — план в `ROADMAP.md`**. Сделано 2026-06-11: ✅ rail v2.1 (flow-блок текущей части в потоке списка, H3-группы под активным H2, плавная metro-линия, дедуп track) — ✅ §0.1 честный localStorage-прогресс + галочки + празднование, ✅ §0.2 resume-тост, ✅ §0.3 «осталось ~N мин», ✅ §0.4 клавиатура ←/→ (+VT), ✅ §0.5 свайп+peek+онбординг, ✅ §0.6 автоскрытие моб. баров, ✅ §0.7 onerror-фолбэки обложек/hero, ✅ §0.8 sum-точки серии. Далее: §1 общесайтовый слой (TTS, крошка-прогресс, стихи, оригинал), §2 бэклог selected-pack |
...
**Лендинг /hard-texts/ (64b3d20c):** получил GBS-карту серии
(`.gbs2-timeline` 3 колонки) + стартовую карточку `.gbs2-next`.
Для этого введён **`.gbs2-scope`** — класс, дающий gbs2-переменные
(light+dark) на страницах БЕЗ body.gbs-world. Используй его, если нужно
встроить gbs2-компонент на каталог/лендинг. Также синхронизированы
минуты чтения krajne по всему сайту (32→41) и статистика лендинга (50→59).
...
## 3. План — ВЕСЬ ВЫПОЛНЕН (история коммитов)
...
5. ✅ Сквозной Playwright-проход всех 5 страниц + фикс themeToggle (1502d865).
6. ✅ AGENTS.md r96 + handoff.
7. ✅ Hard-texts по «да» владельца: обе статьи (c1e69cb3) + минуты 32→41
   site-wide (b1a49c29) + лендинг с картой серии и .gbs2-scope (64b3d20c) + r97/r98.
8. ✅ Финальное закрытие (0a0fa181): снят мёртвый series-cards.js с 7 страниц,
   interactive-audit переписан под GBS-контракт (series: 7 green),
   **G113 gbsWorldIntegrityGuard** в audit-pro (full kit + zero legacy на
   каждой gbs-world странице), readable-audit учит --gbs2-cover,
   AGENTS §9.11 переписан (GBS = канон), OWNER-REQUIREMENTS п.11 обновлён.

## 3a. Что осталось ВЛАДЕЛЬЦУ решить (агентам без «да» не трогать)

- «Тёмная сторона кафедры» (pastor-series): 1 статья — GBS осмыслен при ≥2 частях.
- «Римлянам 8»: когда написана → миграция по PATTERN.md
  (done=59, total пересчитать, обновить series.json + обе сестринские страницы).
- `js/series-cards.js`: режимы strip/nav теперь мёртвый код (страниц-хостов нет),
  живым остался только data-series-cards для каталогов. Можно вычистить
  строки strip/nav при следующем JS-пассе — НО только с прогоном
  interactive-audit + audit-pro (G109 проверяет этот файл).
- Удалить эту папку `_agent-handoff/` когда владелец сочтёт раскатку закрытой.

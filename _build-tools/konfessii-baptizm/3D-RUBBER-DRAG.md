...
## Доп. визуал «шедевр» (r120)
- **Глубокий космос** (`buildCosmos`, рядом с getGlowSprite; вызов в scene-setup
  useEffect после `scene.fog=`): 3 слоя звёзд (Points, параллакс по глубине,
  additive) + 3 туманности-спрайта (золото/индиго/синь) по краям. Только тёмная
  тема, строится один раз (флаг `__cosmosBuilt`).
- **Символика крещения** (faith-узел = Н. Воронин, крещён в реке Кура): под крестом
  3 кольца `baptismRipple`, в RAF-loop расходятся вверх и затухают (волны на воде).

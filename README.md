...
## Правила атрибуции — Редактор: Фёдор Милованов

На сайте Фёдор Милованов является **редактором**, а не «автором» в традиционном смысле: он задаёт направление, редактирует текст, исправляет неточности и собирает информацию при помощи ИИ. Это отражается везде единообразно.

### Три типа статей

| Тип | Описание | Byline | author-card-label |
|-----|----------|--------|-------------------|
| **A** | Авторская статья | `Редактор: Фёдор Милованов` | `Редактор` |
| **B** | Авторская серия / разбор | `Редактор: Фёдор Милованов` | `Редактор` |
| **C** | Перевод зарубежной статьи | `Редакция перевода: Фёдор Милованов` | `Редакция перевода` |
...
3. **`.article-byline__strong`** в `<header>` — полный ярлык: `Редактор: Фёдор Милованов` (Тип A/B) или `Редакция перевода: Фёдор Милованов` (Тип C).
4. **`.author-card-label`** в `<aside class="author-card">` — сокращённо: `Редактор` (Тип A/B) или `Редакция перевода` (Тип C).
5. **Карточки на `index.html` и `articles/index.html`** — `Редактор: Фёдор Милованов` (Тип A/B) или `Ред. перевода: Фёдор Милованов` (Тип C).
...
> **Важно:** нигде не использовать слово «Автор» применительно к Фёдору Милованову в контексте конкретной статьи — только «Редактор» (или «Редакция перевода» для Типа C). Страница `/about/` и SEO-описания используют «автор и редактор» как общую характеристику роли на сайте — это допустимо.
...
> ⚠️ **Изображения не включены в репозиторий** — все контентные изображения (иллюстрации статей) размещаются отдельно (например, через CDN или локально). OG-изображения для Нагорной проповеди уже присутствуют в `/images/`. Остальные файлы перечислены ниже.
    ...
    <aside class="author-card">
      ...
      <div class="author-card-body">
        ...
        <p class="author-card-desc">
          Редактор проекта «Господь Бог — Сила Моя», Санкт-Петербург.

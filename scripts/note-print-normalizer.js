...
function migrate(file, stages) {
  let changed = false;
  for (const [before, after, label] of stages) changed = normalize(file, before, after, label) || changed;
  return changed;
}

const changed = [
  migrate(REGISTRY, [
    [CSS_V1, CSS_V2, 'NoteRegistry print CSS v2'],
    [CSS_V2, CSS_V3, 'NoteRegistry print CSS v3'],
  ]),

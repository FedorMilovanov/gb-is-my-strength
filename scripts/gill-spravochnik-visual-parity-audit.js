...
function assertReferenceStorageContract() {
  ...
  if (resolved.reference.repositoryPath !== storagePath || observedReadPath !== absolutePath) {
    throw new Error('Gill spravochnik reference contract: consumer did not read the resolver-selected physical storage path');
  }
  if (resolved.html !== '<body>resolver-backed-reference</body>') {
    throw new Error('Gill spravochnik reference contract: resolver-backed bytes were not consumed');
  }

  let identityFailure = null;
  try {
    readResolvedLegacy(
      { ...profile, legacyPath: 'articles/wrong-spravochnik/index.html' },
  ...
  if (!identityFailure || !/identity mismatch/.test(String(identityFailure.message))) {
    throw new Error('Gill spravochnik reference contract: profile/ledger identity mismatch did not fail closed');
  }
  ok('Gill spravochnik reference storage contract: resolved physical path + identity fail-closed');
}

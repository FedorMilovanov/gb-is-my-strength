...
function first(source, oldText, newText, label) {
  if (!source.includes(oldText)) throw new Error(`[search-p3-finalizer] ${label}: marker missing`);
  return source.replace(oldText, newText);
}

function section(source, startMarker, endMarker, replacement, label) {
...
function transformSearch(source) {
  ...
  if (source.includes('Promise.all(n.map(function(e){return e.data()}))')) {
    source = once(source, 'Promise.all(n.map(function(e){return e.data()}))', '__gbHydratePagefind(n,t)', 'replace unbounded Pagefind hydration');
  }
  if (source.includes('E.addEventListener("input",function(){A=E.value.trim(),L.style.display=')) {
    source = once(
      ...
      'E.addEventListener("input",function(){A=E.value.trim(),++M,__gbClearMore(),L.style.display=',
      'invalidate stale async work and continuation on input',
    );
  }
  if (source.includes('L.addEventListener("click",function(){E.value="",A="",L.style.display=')) {
    source = once(
      ...
      'invalidate stale async work on clear',
    );
  }
...
function transformContract(source) {
  ...
  source = once(
    ...
    'parameterize continuation fixture viewport',
  );
  source = once(
    source,
    '    const context = await browser.newContext({ viewport: { width: 960, height: 760 } });',
    '    const context = await browser.newContext({ viewport });',
    'use requested continuation viewport',
  );
  source = once(
    source,
  ...
  source = once(
    ...
    "      summary.pagefind = await assertPaged(page, 27, 'рез.');\n      await context.close();\n    }" + staleRace,
    'insert stale query race contract',
  );
  ...
  source = once(
    ...
    'run continuation on full desktop/mobile engine matrix',
  );
  source = once(
    source,
    ...
    'record continuation assertions',
  );
  source = once(
    source,
    ...
    'document continuation coverage',
  );
  return source;
}
...
for (const [marker, label] of [
  ...
  ['__gbHydratePagefind(n,t)', 'bounded Pagefind call'],
  ['A=E.value.trim(),++M,__gbClearMore()', 'input generation invalidation'],
...
]) requireMarker(searchSource, marker, label);
if (searchSource.includes('Promise.all(n.map(function(e){return e.data()}))')) throw new Error('[search-p3-finalizer] unbounded Pagefind hydration survived');

for (const [marker, label] of [
  ["window.dispatchEvent(new CustomEvent('gb:openSearch'))", 'public lazy-open fixture'],
  ['let phase = \'setup\';', 'preserved continuation diagnostics'],
  ['continuation-failure-', 'preserved failure artifacts'],
  ['Array.from({ length: 28 }, (_, index) => index < 2', 'Pagefind multi-window fixture'],
  ["summary.pagefind = await assertPaged(page, 27, 'рез.');", 'Pagefind total'],
  ["summary.fallback = await assertPaged(page, 25, 'рез.');", 'fallback total'],
  ["summary.scripture = await assertPaged(page, 25, 'вх.');", 'Scripture total'],
  ["phase = 'stale-query';", 'stale race diagnostics phase'],
  ['__searchP3RaceStarted', 'stale async fixture'],
  ['continuationMultiStep: true', 'continuation report assertion'],
  ['runContinuationContract(browserType, browserName, port, viewport)', 'full continuation matrix'],
]) requireMarker(contractSource, marker, label);
if (contractSource.includes("await page.waitForFunction(() => window.GBSearch && typeof window.GBSearch.open === 'function');")) throw new Error('[search-p3-finalizer] pre-open GBSearch wait survived');

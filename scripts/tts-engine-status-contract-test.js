...
function validateLiveDeploymentContract(liveContract, deployWorkflow) {
  ...
  const checks = [
    ...
    ['deploy passes exact direct SHA', deployWorkflow, /DEPLOYED_SHA:\s*\$\{\{ github\.sha \}\}/],
...
function validateDeploymentProvenance(library, writer, liveContract, deployWorkflow) {
  ...
  const checks = [
    ...
    ['library writes schema v3 generic manifest', library, /schemaVersion:\s*3[\s\S]*artifact:[\s\S]*build:[\s\S]*criticalAssets,[\s\S]*extensions:/],
    ['library stores TTS below extensions', library, /extensions:\s*\{[\s\S]*tts:\s*\{/],
    ['writer requires exact release identity', writer, /prepareReleaseCandidate[\s\S]*GITHUB_RUN_ID[\s\S]*GITHUB_RUN_ATTEMPT/],
    ...
    ['deploy writes provenance before candidate upload', deployWorkflow, /- name: Write generic immutable release provenance[\s\S]*node scripts\/write-deployment-provenance\.mjs[\s\S]*- name: Upload immutable release candidate/],
    ...
    ['live contract compares schema v3 TTS extension', liveContract, /deployment provenance schema drifted[\s\S]*deployment provenance TTS extension mismatch/],
  ];
  for (const [label, source, pattern] of checks) if (!pattern.test(source)) problems.push(label);
...
for (const [name, libraryMutation, writerMutation, liveMutation, deployMutation] of [
  ...
  ['writer candidate output removed', releaseLibrary, provenanceWriter.replace('candidate_digest=${report.digest}', 'candidate_digest=unchecked'), liveDeploymentContract, deployWorkflow],
  ['deploy provenance step removed', releaseLibrary, provenanceWriter, liveDeploymentContract, deployWorkflow.replace('node scripts/write-deployment-provenance.mjs', 'echo provenance skipped')],
  ...
  ['live run-addressed path removed', releaseLibrary, provenanceWriter, liveDeploymentContract.replace('provenancePath: `/deployments/${DEPLOYED_SHA}/${runIdentity}.json`', "provenancePath: '/deployment.json'"), deployWorkflow],
...
console.log('TTS engine status contract: PASS (' + (mutations.length + 17) + ' named adversarial mutations rejected).');

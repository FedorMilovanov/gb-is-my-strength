...
assert.match(commitSha, /^[a-f0-9]{40}$/, 'DEPLOYED_SHA must be exact');
...
function probeUrl(relative, attempt, label) {
  ...
  target.searchParams.set('release_contract', `${commitSha.slice(0, 12)}-${attempt}-${label}-${Date.now()}`);
  return target;
}
async function fetchBuffer(relative, attempt, label) {
  ...
  const response = await fetch(target, {
    ...
    headers: {
      ...
      pragma: 'no-cache',
      'user-agent': 'gb-live-release-contract/1.0',
    },
...
function assertPointer(pointer) {
  assert.equal(pointer.schemaVersion, 2, 'live pointer schema drifted');
  assert.equal(pointer.repository, repository, 'live pointer repository mismatch');
  assert.equal(pointer.commitSha, commitSha, 'live pointer SHA mismatch');
...
function assertManifest(manifest) {
  assert.equal(manifest.schemaVersion, 3, 'live release manifest schema drifted');
  assert.equal(manifest.repository, repository, 'live release repository mismatch');
  assert.equal(manifest.commitSha, commitSha, 'live release SHA mismatch');
...
for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
  ...
  try {
    ...
    writeReport();
    console.log(`Live release contract: PASS (${commitSha}, ${expectedDigest}).`);
    process.exit(0);
  } catch (error) {
    lastError = error;
    entry.result = 'FAIL';

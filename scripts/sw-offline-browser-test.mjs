...
const siteUtilsRevision = swSource.match(/['"]\/js\/site-utils\.js\?v=([a-f0-9]{8})['"]/)?.[1];
assert.ok(siteUtilsRevision, 'dist/sw.js must precache the governed site-utils revision');
const exactSiteUtilsUrl = `/js/site-utils.js?v=${siteUtilsRevision}`;
const unseenSiteUtilsRevision = siteUtilsRevision === 'deadbeef' ? 'feedface' : 'deadbeef';
const expectedSiteUtilsSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(DIST, 'js', 'site-utils.js'))).digest('hex');
...
function count(pathname) { state.requestCounts.set(pathname, (state.requestCounts.get(pathname) || 0) + 1); }
...
async function releaseMarker(page) { return page.locator('meta[name="a07-release"]').getAttribute('content'); }
async function responseDigest(page, url) {
  return page.evaluate(async (target) => {
    ...
    return { status: response.status, length: bytes.length, sha256, text: new TextDecoder().decode(bytes.slice(0, 200)) };
  }, url);
}
async function rejectedFetch(page, url) {
  return page.evaluate(async (target) => {
    try {
      const response = await fetch(target);
...
try {
  ...
  const partial = await partialPage.evaluate(async () => {
    ...
    return { state: worker && worker.state, active: Boolean(registration.active), controller: Boolean(navigator.serviceWorker.controller), caches: await caches.keys() };
  });
  ...
  pass('partial precache failure cannot activate', 'worker redundant; failed staging-generation cache removed');
  ...
  const staticKeys = await page.evaluate(async (cacheName) => (await (await caches.open(cacheName)).keys()).map((request) => {
    const url = new URL(request.url);
    return url.pathname + url.search;
  }), `${oldVersion}-static`);
  assert.ok(staticKeys.some((key) => key.startsWith('/css/site.css?v=')), 'site.css must retain governed revision in precache');
  assert.ok(staticKeys.includes(exactSiteUtilsUrl), 'site-utils exact governed revision must be precached');
  assert.ok(staticKeys.includes('/pagefind/pagefind.js'), 'Pagefind bootstrap must be precached');
  assert.ok(staticKeys.includes('/404.html'), '404 fallback must be precached');
  assert.equal(staticKeys.some((key) => key.startsWith(mapEnginePath)), false);
  pass('cold atomic install', `${staticKeys.length} complete precache entries with exact governed revisions`);

  await context.setOffline(true);
  const exactRevisionOffline = await responseDigest(page, exactSiteUtilsUrl);
  assert.equal(exactRevisionOffline.status, 200);
  assert.equal(exactRevisionOffline.sha256, expectedSiteUtilsSha, 'exact offline revision must preserve complete byte identity');
  pass('revisioned static exact offline byte identity', `${exactSiteUtilsUrl} -> sha256:${exactRevisionOffline.sha256}`);

  const unseenRevision = await rejectedFetch(page, `/js/site-utils.js?v=${unseenSiteUtilsRevision}`);
  assert.equal(unseenRevision.rejected, true, `unseen revision must fail closed, got ${JSON.stringify(unseenRevision)}`);
  pass('revisioned static cross-generation miss fails closed', `unseen ?v=${unseenSiteUtilsRevision} never falls back to canonical bytes`);
  ...
  assert.equal(pagefindOffline.sha256, pagefindOnline.sha256);
  ...
  pass('Pagefind boundary', 'bootstrap precached; visited data retains byte identity offline');
  await context.setOffline(false);

  const audioOnline = await responseDigest(page, '/audio/a07-model.bin');
  assert.equal(audioOnline.status, 200);
  await context.setOffline(true);
  const audioOffline = await rejectedFetch(page, '/audio/a07-model.bin');
  assert.equal(audioOffline.rejected, true);
  ...
  await page.reload({ waitUntil: 'domcontentloaded' });
  assert.equal(await releaseMarker(page), 'new');
  pass('release update over old cache', 'old generation caches removed only after valid successor activation; new route survives offline');
  ...
  fs.writeFileSync(path.join(REPORT_DIR, 'a07-offline-pwa-browser.md'), [
    '# A07 Offline/PWA Chromium witness', '', `- Cache version: \`${currentVersion}\``,
    `- Exact revision witness: \`${exactSiteUtilsUrl}\` -> \`sha256:${expectedSiteUtilsSha}\``,
    `- Scenarios: **${results.length}/${results.length} passed**`, `- Evidence digest: \`sha256:${digest}\``, '',

...
const exactKeys = (value, expected, label) => {
  ...
  if (!isDeepStrictEqual(actualKeys, expectedKeys)) fail(`${label} keys must be exactly ${expectedKeys.join(', ')}`);
};
...
if (!Array.isArray(provenance.draftArticles) || provenance.draftArticles.length !== 2) fail('exactly two extension bindings required');
if (!Array.isArray(provenance.siteArticles) || provenance.siteArticles.length !== 6) fail('exactly six site contracts required');
...
for (const field of requiredPolicy) {
...
if (research.extension?.schemaVersion !== 3) fail('extension schemaVersion must be 3');
...
if (!isDeepStrictEqual(research.extension?.resolvedByPolicy, EXPECTED_POLICY_RESOLUTIONS)) fail('policy resolution pin drift');
...
if (!process.exitCode) {
  ...
  if (!process.exitCode) {
    ...
    if (extensionManifest.schemaVersion !== 3 || extensionLedger.schemaVersion !== 3) fail('extension schema mismatch');
    ...
    if (!isDeepStrictEqual(extensionManifest.holdRegistry?.resolvedByPolicy, EXPECTED_POLICY_RESOLUTIONS)) fail('Research policy resolution drift');
    ...
    if (manifestAcceptance.publicationAuthorized !== false) fail('Research site acceptance must not publish');

    const release = extensionLedger.releaseDecision || {};
    if (release.state !== 'blocked' || release.mayPublish !== false || release.mayRemoveNoindex !== false) fail('extension release must remain blocked');
    ...
    if (!isDeepStrictEqual(release.resolvedByPolicy, EXPECTED_POLICY_RESOLUTIONS)) fail('ledger policy resolution drift');
...
if (!process.exitCode) {
  console.log(
    `Genesis 6 Research provenance: PASS (${research.commit}, ` +
      ...
      `${research.extension.blockingHolds.length} blocking HOLDs, ${research.extension.resolvedByEvidence.length} evidence resolution, ` +

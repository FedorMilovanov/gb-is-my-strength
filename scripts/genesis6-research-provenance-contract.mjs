...
if (!process.exitCode) {
  ...
  for (const file of [manifestFile, ledgerFile, contractFile]) if (!fs.isFileSync(file)) fail(`missing pinned Research file: ${path.relative(RESEARCH_ROOT, file)}`);
  ...
  if (!process.exitCode) {
    ...
    for (const binding of provenance.articles) {
      ...
      for (const [name, bundle] of [['manifest', manifestBundles.get(binding.bundleId)], ['ledger', ledgerBundles.get(binding.bundleId)]]) {
        ...
        if (JSON.stringify(bundle) !== JSON.stringify(expected)) fail(`${name} bundle drift for ${binding.bundleId}`);
      }
    }

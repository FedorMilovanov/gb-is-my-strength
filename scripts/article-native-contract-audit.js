...
function auditArticle(record, baselineByUrl) {
  ...
  const currentWords = wordCount(articleHtml(html));
  ...
  if (!baseline) {
    ...
    if (oldWords >= 80 && currentWords < floor) {
      const message = `${label}: content floor failed ${oldWords} -> ${currentWords} (floor ${floor})`;
      REQUIRE_CONTENT_PARITY ? bad(message) : warn(message);
    } else {
      ok(`${label}: content floor preserved (${currentWords} words; baseline ${oldWords})`);
    }
  }
}

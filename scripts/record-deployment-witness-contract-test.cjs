...
const TARGET_MARKER = `<!-- deployment-witness-target:release:${SHA} -->`;
const LEGACY_TARGET_MARKER = `<!-- deployment-witness-target:tts:${SHA} -->`;
const COMMENT_MARKER = `<!-- deployment-release-witness:${SHA}:${RUN_ID}:${RUN_ATTEMPT}:${CANDIDATE_ARTIFACT_ID}:${GENERIC_ARTIFACT_ID}:${TTS_ARTIFACT_ID} -->`;
...
function genericReport(overrides = {}) {
  const immutablePath = `/deployments/${SHA}/${RUN_ID}-${RUN_ATTEMPT}.json`;
  const base = {
    ...
    attempts: [{
      ...
      evidence: {
        ...
        immutablePath,
        candidateId: `${SHA}:${RUN_ID}-${RUN_ATTEMPT}`,
...
function createHarness({ artifacts = [candidateArtifact(), genericArtifact(), ttsArtifact()], pulls = [], issues = [], comments = {} } = {}) {
  const calls = { comments: [], updates: [], warnings: [], infos: [], summaries: [] };
  ...
  try {
    {
      ...
      assert.equal(result.envelope.releaseCandidate.candidateId, `${SHA}:${RUN_ID}-${RUN_ATTEMPT}`);
      ...
      assert.ok(harness.calls.comments.every((entry) => entry.body.includes('Release candidate witness accepted')));
    ...
    }
    {
      const witnessDirectory = createWitnessDirectory();
      directories.push(witnessDirectory);
      await assert.rejects(invoke(createHarness({ artifacts: [candidateArtifact({ digest: 'sha256:bad' }), genericArtifact(), ttsArtifact()] }), witnessDirectory), /release candidate artifact digest is missing or invalid/);
    }
    {
    ...
    {
      const witnessDirectory = createWitnessDirectory({ tts: ttsReport({ candidateDigest: `sha256:${'e'.repeat(64)}` }) });
      directories.push(witnessDirectory);
    ...
    }
    {
      ...
      const harness = createHarness({
        ...
        issues: [{ number: 101, title: 'Partial', body: `deployment-witness-target:release:${SHA.slice(0, 7)}`, state: 'open' }],
      });
      const result = await invoke(harness, witnessDirectory);
    ...
    console.log('Deployment release witness contract: PASS (candidate transport, generic live, TTS extension, exact PR/SHA, idempotency, ambiguity and scope).');
  } finally {

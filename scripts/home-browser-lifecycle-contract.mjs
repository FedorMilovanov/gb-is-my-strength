...
function isKnownNavigationAbort(request) {
  if (request.failure()?.errorText !== 'net::ERR_ABORTED' || request.resourceType() !== 'script') return false;
  try {
    return new URL(request.url()).pathname === '/pagefind/pagefind.js';
  } catch {
    return false;
  }
}
...
async function runBrowser(browserName, browserType, baseUrl) {
  ...
  page.on('requestfailed', (request) => {
    ...
    if (isKnownNavigationAbort(request)) ignoredDiagnostics.push(diagnostic);

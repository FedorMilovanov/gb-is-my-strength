...
const DEPLOYED_SHA = String(process.env.DEPLOYED_SHA || process.env.GITHUB_SHA || '').trim().toLowerCase();
...
const local = verifyReleaseCandidate({
  ...
  expectedCommitSha: DEPLOYED_SHA,
...
async function fetchBuffer(relative, attempt, label) {
  ...
  const response = await fetch(target, {
    ...
    headers: { 'cache-control': 'no-cache, no-store, max-age=0', pragma: 'no-cache', 'user-agent': 'gb-tts-live-deployment-contract/4.0' },
...
function assertPointer(pointer) {
  assert.equal(pointer.schemaVersion, 2, 'deployment current pointer schema drifted');
...
function assertProvenance(manifest) {
  assert.equal(manifest.schemaVersion, 3, 'deployment provenance schema drifted');
...
async function verifyAttempt(attempt) {
  ...
  return {
    discovery: { path: expected.currentPointerPath, immutablePath: pointer.immutablePath, workflowRunId: pointer.workflow.runId, workflowRunAttempt: pointer.workflow.runAttempt, candidateDigest: pointer.artifact.digest },
    provenance: { path: expected.provenancePath, commitSha: provenance.commitSha, workflowRunId: provenance.workflow.runId, workflowRunAttempt: provenance.workflow.runAttempt, sourceReadinessRunId: provenance.workflow.runId, candidateDigest: provenance.artifact.digest },
    routeEvidence,
    assets: {
...
for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt += 1) {
  ...
  try {
    ...
    writeReport();
    console.log(`TTS live deployment contract: PASS (${DEPLOYED_SHA}, run ${runIdentity}, candidate ${EXPECTED_CANDIDATE_DIGEST}).`);
    process.exit(0);
  } catch (error) {
    lastError = error;
    entry.result = 'FAIL';

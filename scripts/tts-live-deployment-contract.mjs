...
const DEPLOYED_SHA = String(process.env.DEPLOYED_SHA || process.env.GITHUB_SHA || 'unknown');
...
const local = {
  controller: readBuffer(LOCAL_FILES.controller),
  engine: readBuffer(LOCAL_FILES.engine),
  noticeCss: readBuffer(LOCAL_FILES.noticeCss),
};

const expected = Object.freeze({
  controllerRevision: md5(local.controller),
  engineRevision: md5(local.engine),
  noticeCssRevision: md5(local.noticeCss),
  controllerPath: `/js/floating-cluster-controller.js?v=${md5(local.controller)}`,
  enginePath: `/js/vosk-tts-engine.js?v=${md5(local.engine)}`,
  noticeCssPath: `/css/tts-download-notice.css?v=${md5(local.noticeCss)}`,
});
...
async function fetchBuffer(url, attempt, label) {
  ...
  const response = await fetch(target, {
    ...
    headers: {
      ...
      pragma: 'no-cache',
      'user-agent': 'gb-tts-live-deployment-contract/1.0',
    },
...
function assertLocalRevisionChain() {
  const controllerText = local.controller.toString('utf8');
  const engineText = local.engine.toString('utf8');
  assert.ok(controllerText.includes(expected.enginePath), `local controller does not own ${expected.enginePath}`);
  assert.ok(controllerText.includes(expected.noticeCssPath), `local controller does not own ${expected.noticeCssPath}`);
  assert.ok(engineText.includes(expected.noticeCssPath), `local engine does not own ${expected.noticeCssPath}`);
}
...
async function verifyAttempt(attempt) {
  ...
  return {
    routeEvidence,
    assets: {
      controller: { path: expected.controllerPath, revision: md5(controller.buffer) },
      engine: { path: expected.enginePath, revision: md5(engine.buffer) },
      noticeCss: { path: expected.noticeCssPath, revision: md5(noticeCss.buffer) },
      serviceWorker: { url: sw.url, lazyTtsPrecache: false },
    },
  };
}

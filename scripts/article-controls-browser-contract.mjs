...
async function tagRenderedControls(page) {
  await page.evaluate(() => {
    ...
    [...document.querySelectorAll('button,[role="button"]')].filter(rendered).forEach((e, index) => e.setAttribute('data-gb-audit-control-index', String(index)));
  });
}
...
async function snapshot(page) {
  ...
  return page.evaluate(() => {
    ...
    const controls = [...document.querySelectorAll('button,[role="button"]')].filter(rendered).map((e, auditIndex) => {
      ...
      return {
        auditIndex,id:e.id||'',name:name(e).slice(0,160),text:(e.textContent||'').replace(/\s+/g,' ').trim().slice(0,100),cls:cls.slice(0,140),
        ...
        owns:!inView||!!(hit&&(hit===e||e.contains(hit))),inline,specialized,
      };
    });
...
async function clickOne(page, route, c) {
  ...
  return page.evaluate(({ controlId, ariaControls, auditIndex }) => {
    ...
    return {ok:true,url:location.href,dialogs,openText,cp:!!cp,cpInternal:cpLinks.filter(h=>h.startsWith('/')||h.startsWith('.')||h.startsWith('#')).length,
...
function appendProgress(scene, completed, expected) {
  ...
  fs.writeFileSync(CHECKPOINT,`${JSON.stringify({schemaVersion:1,sourceSha:process.env.SOURCE_SHA||'',completedScenes:completed,expectedScenes:expected,last:compact},null,2)}\n`);
...
async function scene(browser,browserName,route,view,doClicks) {
  ...
  try {
    ...
    for(const c of snap.controls){if(!c.name&&!c.inline)baseIssues.push(['control-no-accessible-name',c]);if(!c.inline&&(c.w<24||c.h<24))baseIssues.push(['small-control-target',c]);if(!c.inline&&c.clipped)baseIssues.push(['control-clipped',c]);if(!c.inline&&c.inView&&!c.owns)baseIssues.push(['control-center-obscured',c]);}
    issues.push(...baseIssues);for(const [kind] of baseIssues)await maybeScreenshot(page,kind,route,browserName,viewName);
    if(doClicks){for(const c of snap.controls){if(!clickable(c))continue;const o=await clickOne(page,route,c);clicks.push({i:c.auditIndex,name:c.name.slice(0,80),ok:o.ok,url:o.ok?new URL(o.url).pathname:'',cp:o.ok?o.cp:false,expanded:o.ok?o.triggerExpanded:null});for(const [kind,detail] of semanticIssues(c,o,route)){issues.push([kind,{control:{id:c.id,name:c.name,cls:c.cls,ariaControls:c.ariaControls},detail}]);await maybeScreenshot(page,kind,route,browserName,viewName);}if(o.ok)for(const d of o.dialogs){const[l,t,r,b]=d.rect;if(l<-2||t<-2||r>width+2||b>height+2){issues.push(['dialog-outside-viewport',{control:{id:c.id,name:c.name},dialog:d}]);await maybeScreenshot(page,'dialog-outside-viewport',route,browserName,viewName);}}}}
...
async function main(){
  ...
  const routes=discoverRoutes(),reps=[];for(const f of ['hermenevtika','gill','baptisty','hard-texts','nagornaya','articles']){const r=routes.find(x=>family(x)===f);if(r)reps.push(r)}
  ...
  const wb=await webkit.launch({headless:true});try{for(const r of reps)for(const v of VIEWS.filter(x=>CLICK_VIEWS.has(x[0]))){const s=await scene(wb,'webkit',r,v,true);scenes.push(s);appendProgress(s,++completed,expected);}}finally{await wb.close()}
  ...
  const summary={schemaVersion:4,sourceSha:process.env.SOURCE_SHA||'',generatedAt:new Date().toISOString(),routeCount:routes.length,sceneCount:scenes.length,controlObservations:scenes.reduce((n,s)=>n+s.controls,0),controlClicks:scenes.reduce((n,s)=>n+s.clicks.length,0),specializedInlineSkipped:scenes.reduce((n,s)=>n+s.skippedSpecialized,0),webkitRepresentatives:reps,issueCount:manifestations.length,issueKinds,issues:manifestations};
  ...
  if(manifestations.length){console.log(`❌ ${manifestations.length} issue manifestation(s)`);for(const i of manifestations.slice(0,180))console.log(`- ${i.kind} ${i.browser}/${i.view} ${i.route}: ${JSON.stringify(i.detail)}`);process.exitCode=1}else console.log('✅ Article control census passed');
}

...
updateFile(paginationPath, (text) => {
  ...
  if (!text.includes('function markTerminalRegion(')) {
    const helperAnchor = `  function classifyCandidates(root, pageHeight) {`;
    if (!text.includes(helperAnchor)) throw new Error('terminal-region helper insertion point missing');
    text = text.replace(
      helperAnchor,
  ...
  const oldTerminalCss = `      '  html body [data-print-terminal-flow] { break-after: auto !important; page-break-after: auto !important; }',`;
  const terminalCss = `      '  html body [data-print-terminal-flow] { break-before: auto !important; page-break-before: auto !important; break-after: auto !important; page-break-after: auto !important; }',`;
  if (text.includes(oldTerminalCss)) text = text.replace(oldTerminalCss, terminalCss);
  if (!text.includes(terminalCss)) {
    if (!text.includes(cssAnchor)) throw new Error('terminal-flow CSS insertion point missing');
    text = text.replace(cssAnchor, `${terminalCss}\n${cssAnchor}`);
  }

  const clearAnchor = `      nodes[i].removeAttribute('data-print-closing-group');`;
  if (!text.includes("nodes[i].removeAttribute('data-print-terminal-flow');")) {
    if (!text.includes(clearAnchor)) throw new Error('terminal-flow cleanup insertion point missing');
    text = text.replace(clearAnchor, `${clearAnchor}\n      nodes[i].removeAttribute('data-print-terminal-flow');`);
  }

  const oldStats = `var stats = { candidates: candidates.length, atomic: 0, splittable: 0, rows: 0, keepNext: 0, tailPairs: 0, closingGroups: 0, tails: 0 };`;
  const newStats = `var stats = { candidates: candidates.length, atomic: 0, splittable: 0, rows: 0, keepNext: 0, tailPairs: 0, closingGroups: 0, terminalFlow: 0, tails: 0 };`;
  if (text.includes(oldStats)) text = text.replace(oldStats, newStats);
  if (!text.includes(newStats)) throw new Error('terminal-flow stats anchor missing');

  const oldGroup = `        if (group) {\n          var terminal = group.parentElement;\n          while (terminal && terminal !== document.body && terminal !== document.documentElement) {\n            mark(terminal, 'data-print-terminal-flow', '1');\n            terminal = terminal.parentElement;\n          }\n          stats.tailPairs += 1;`;
  const baseGroup = `        if (group) {\n          stats.tailPairs += 1;`;
  const newGroup = `        if (group) {\n          stats.terminalFlow += markTerminalRegion(group, scope);\n          stats.tailPairs += 1;`;
  if (text.includes(oldGroup)) text = text.replace(oldGroup, newGroup);
  else if (text.includes(baseGroup)) text = text.replace(baseGroup, newGroup);
  if (!text.includes(newGroup)) throw new Error('terminal-region grouping insertion point missing');
  return text;
});

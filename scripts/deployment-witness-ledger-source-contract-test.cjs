...
function validate({ deploy, ledger, workflow, recorder }) {
  ...
  const checks = [
    ...
    ['readiness exposes two SHAs', releaseReadiness, /release_sha:\s*\$\{\{ steps\.provenance\.outputs\.release_sha \}\}[\s\S]*control_plane_sha:\s*\$\{\{ steps\.provenance\.outputs\.control_plane_sha \}\}/],
    ['deploy consumes both SHAs', releaseDeploy, /EXPECTED_RELEASE_SHA:\s*\$\{\{ needs\.readiness\.outputs\.release_sha \}\}[\s\S]*EXPECTED_CONTROL_PLANE_SHA:\s*\$\{\{ needs\.readiness\.outputs\.control_plane_sha \}\}/],
    ...
    ['resolver exports canonical control identity', resolver, /outputs:[\s\S]*run_id:\s*\$\{\{ steps\.resolve\.outputs\.run_id \}\}[\s\S]*control_plane_sha:\s*\$\{\{ steps\.resolve\.outputs\.control_plane_sha \}\}[\s\S]*recorder_ref:\s*\$\{\{ steps\.resolve\.outputs\.recorder_ref \}\}/],
    ...
    ['resolver asserts full control SHA', resolver, /requested deploy run lacks an exact control-plane SHA/],
    ['resolver emits API control SHA', resolver, /core\.setOutput\('control_plane_sha', workflowRun\.head_sha\);/],
    ...
    ['writer revalidates control-plane SHA', writer, /EXPECTED_DEPLOY_CONTROL_PLANE_SHA[\s\S]*writer target deploy control-plane SHA mismatch/],
    ...
    ['recorder validates artifact control SHA', recorder, /artifact\.workflow_run\.head_sha[\s\S]*artifact control-plane SHA mismatch/],
    ['recorder derives exact release identity', recorder, /const releaseSha = normalize\(generic\.releaseSha\)\.toLowerCase\(\)[\s\S]*assert\.match\(releaseSha, FULL_SHA_RE, 'generic release witness release SHA is missing or invalid'\)/],
    ['recorder validates generic PASS and control identity', recorder, /assert\.equal\(generic\.result, 'PASS'[\s\S]*assert\.equal\(generic\.controlPlaneSha, controlPlaneSha, 'generic release witness control-plane SHA mismatch'\)[\s\S]*assert\.equal\(generic\.workflowRunId, runId/],
    ['recorder binds generic release evidence', recorder, /assert\.equal\(genericEvidence\.releaseSha, releaseSha, 'generic release evidence release SHA mismatch'\)[\s\S]*assert\.equal\(genericEvidence\.controlPlaneSha, controlPlaneSha, 'generic release evidence control-plane SHA mismatch'\)/],
    ...
    ['recorder binds TTS to both identities', recorder, /TTS witness release SHA mismatch[\s\S]*TTS witness control-plane SHA mismatch[\s\S]*TTS witness candidate digest mismatch/],
    ['recorder generic envelope exact', recorder, /schemaVersion:\s*3[\s\S]*releaseSha,[\s\S]*controlPlaneSha,[\s\S]*releaseCandidate:[\s\S]*liveWitnessArtifact:[\s\S]*extensions:[\s\S]*tts:/],
...
const mutations = [
  ...
  ['resolver control output aliased', { ...sources, ledger: sources.ledger.replace("core.setOutput('control_plane_sha', workflowRun.head_sha);", "core.setOutput('control_plane_sha', context.sha);") }],
  ...
  ['release/control mix accepted', { ...sources, recorder: sources.recorder.replace('generic release witness control-plane SHA mismatch', 'generic identity unchecked') }],
...
console.log(`Deployment release ledger source contract: PASS (${mutations.length} named mutations rejected with independent release/control identity).`);

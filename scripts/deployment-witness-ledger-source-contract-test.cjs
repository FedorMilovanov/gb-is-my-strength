...
function validate({ deploy, ledger, workflow, recorder }) {
  ...
  const checks = [
    ...
    ['recorder validates generic two-SHA PASS identity', recorder, /generic\.result, 'PASS'[\s\S]*generic\.releaseSha[\s\S]*generic\.controlPlaneSha, controlPlaneSha[\s\S]*generic\.workflowRunId, runId/],

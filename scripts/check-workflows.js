...
must(deployPath, deploy, /RELEASE_SHA:\s*\$\{\{ github\.event_name == 'workflow_dispatch' && inputs\.release_sha \|\| github\.sha \}\}/, 'release SHA must remain independently selectable for recovery');
...
must(deployPath, releaseReadiness, /git fetch --no-tags origin "\+main:refs\/remotes\/origin\/main"/, 'candidate job must fetch exact current-main control plane');
must(deployPath, releaseReadiness, /git rev-parse --verify 'HEAD\^\{commit\}'[\s\S]{0,100}= "\$RELEASE_SHA"/, 'candidate job must verify the exact checked-out release commit');
...
must(deployPath, releaseReadiness, /git diff --exit-code/, 'candidate validation must leave tracked release source clean');
must(deployPath, releaseReadiness, /Stage immutable verification tools from trusted control plane[\s\S]*git show "\$\{CONTROL_PLANE_SHA\}:scripts\/\$\{file\}"/, 'candidate verifier tools must come from control-plane SHA');
must(deployPath, releaseReadiness, /release-tools\/write-deployment-provenance\.mjs/, 'candidate job must use trusted generic provenance writer');
must(deployPath, releaseReadiness, /EXPECTED_RELEASE_SHA:\s*\$\{\{ env\.RELEASE_SHA \}\}[\s\S]*EXPECTED_CONTROL_PLANE_SHA:\s*\$\{\{ env\.CONTROL_PLANE_SHA \}\}/, 'candidate verification must bind both SHAs');
must(deployPath, releaseReadiness, /release_sha:\s*\$\{\{ steps\.provenance\.outputs\.release_sha \}\}[\s\S]*control_plane_sha:\s*\$\{\{ steps\.provenance\.outputs\.control_plane_sha \}\}/, 'readiness outputs must expose both identities');
...
must(deployPath, releaseDeploy, /EXPECTED_RELEASE_SHA:\s*\$\{\{ needs\.readiness\.outputs\.release_sha \}\}[\s\S]*EXPECTED_CONTROL_PLANE_SHA:\s*\$\{\{ needs\.readiness\.outputs\.control_plane_sha \}\}/, 'privileged deploy must verify both identities');
...
console.log('✅ Release and control-plane identities are independently verified');
console.log('✅ Privileged Pages actions match the exact approved SHA allowlist');

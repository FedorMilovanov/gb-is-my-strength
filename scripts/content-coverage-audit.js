...
//
// Regression class: CONTENT-PARITY-LOSS-01. Routes owned by Astro keep a
// legacy shadow copy in the repo root; edits landing on the shadow (or content
// silently dropped during componentization) never reach production, and
// visual-parity audits check markers, not text. This gate compares the WORD
// MULTISET of each legacy shadow page against the built dist page and fails
// when a meaningful share of legacy words is missing from dist.
//
// Thresholds: warn > WARN_PCT, fail > FAIL_PCT of legacy words missing.
// Per-route allowlist below documents intentional divergences.
//
...
// Routes where dist intentionally diverges from the legacy shadow.
// Every entry MUST carry a reason; remove entries when shadows are deleted.
const ALLOW = new Map([
...
// Use ONLY for pages whose legacy shadow contains client-rendered UI text or
// in-SVG labels that the native app renders at runtime; the narrative/content
// layer must still be statically present.
...
for (const [route, meta] of Object.entries(ownership)) {
  ...
  if (!fs.existsSync(legacyPath)) { skipped++; continue; } // no shadow — nothing to compare
  ...
  if (pct > FAIL_PCT) {
    bad(`${route}: ${pct.toFixed(1)}% legacy words missing from dist (${missing}/${total}); sample: ${missingWords.join(', ')}`);
  } else if (pct > WARN_PCT) {
    warn(`${route}: ${pct.toFixed(1)}% legacy words missing from dist (${missing}/${total}); sample: ${missingWords.join(', ')} — review; fail threshold ${FAIL_PCT}%`);
...
console.log(`\nContent coverage audit: ${covered} routes compared, ${skipped} skipped, ${warns} warnings, ${checks - failures}/${checks} checks passed`);

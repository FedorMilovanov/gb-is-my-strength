...
function fixture({ variable = 'inst', guard = true, engineGuard = true, guardAfterReady = false } = {}) {
  ...
  const tail = guardAfterReady ? `${ready}\n${nullGuard}` : `${nullGuard}\n${ready}`;
  return `${engine}\nvar ${variable} = window.MapEngine.createMap(container, route, {});\n${tail}`;
}

function run() {
  for (const variable of ['inst', 'instance', 'mapResult']) {
    const result = inspectMapInitSource(fixture({ variable }));
    assert.equal(result.engineGuard, true, `${variable}: engine guard must be detected`);
  ...
  assert.equal(hasFailClosedMapInit(fixture({ guard: false })), false, 'missing null guard must fail');
  ...
  console.log('✅ Map init source contract regression: rename-safe and guard-removal fail-closed');
}

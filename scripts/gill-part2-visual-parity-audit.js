...
const LEGACY_REL = 'articles/dzhon-gill-chast-2-uchenyi/index.html';
const PAGE_REL = 'src/pages/articles/dzhon-gill-chast-2-uchenyi/index.astro';
const BASE_REL = 'src/components/article-pilots/gill-part2';
const REQUIRED = {
  pageHead: `${BASE_REL}/GillPart2PageHead.astro`,
  pageChrome: `${BASE_REL}/GillPart2PageChrome.astro`,
  shell: `${BASE_REL}/GillPart2MainShell.astro`,
  header: `${BASE_REL}/GillPart2HeaderHero.astro`,
  body: `${BASE_REL}/GillPart2ArticleBody.astro`,
  post: `${BASE_REL}/GillPart2PostArticle.astro`,
};
...
function normalize(html){ return String(html||'').replace(/\s+is:inline(?=\s|>)/g,'').replace(/\s+data-pagefind-body(?=\s|>)/g,'').replace(/viewBox=/g,'viewbox=').replace(/\/>/g,'>').replace(/>\s+</g,'><').replace(/\s+/g,' ').trim(); }
console.log('GILL PART II STRICT-NATIVE AUDIT');
...
if (!problems.length) {
  const page = read(PAGE_REL); const pageHead = read(REQUIRED.pageHead); const pageChrome = read(REQUIRED.pageChrome); const shell = read(REQUIRED.shell); const header = read(REQUIRED.header); const body = read(REQUIRED.body); const post = read(REQUIRED.post);
  const legacy = read(LEGACY_REL); const legacyBody = bodyInner(legacy); const bodyMatch = body.match(/<article\b[^>]*class="article-body"[^>]*>[\s\S]*?<\/article>/i); const article = bodyMatch ? bodyMatch[0] : body; const main = `<main id="main-content">${header}${article}${post}</main>`; const reconstructed = pageChrome.replace('<slot />', main); const scopeText = [page,pageHead,pageChrome,shell,header,body,post].join('\n');
  for (const token of FORBIDDEN) mustNotContain('strict-native source scope', scopeText, token);
  mustContain('route imports native page head', page, 'GillPart2PageHead');
  mustContain('route imports native page chrome', page, 'GillPart2PageChrome');
  mustContain('route imports native main shell', page, 'GillPart2MainShell');
  mustContain('route sets explicit body class', page, 'class="gbs-world"');
  mustContain('route sets explicit done minutes', page, 'data-gbs2-done-min="48"');
  mustContain('route sets explicit part minutes', page, 'data-gbs2-part-min="39"');
  ...
  for (const marker of ['class="gbs2-hero"', 'Джон Гилл (1697–1771)', 'id="part-theology"', 'id="sec-trinity"', 'id="sec-hebrew"', 'id="sec-commentary"', 'id="sec-systematics"', 'id="part-controversy"', 'id="sec-whitby"', 'id="sec-quiz"', 'gbs2-next', 'gbs2-timeline', 'article-end-sdg-wrap']) mustContain('reconstructed body marker', reconstructed, marker);
...
console.log('');
if (problems.length) { console.log(`❌ Gill Part II strict-native audit failed: ${problems.length} issue(s)`); process.exit(1); }
console.log('✅ Gill Part II strict-native audit passed');

...
function assertExactIdentity({ repository, releaseSha, controlPlaneSha, runId, runAttempt }) {
  assert.match(repository, /^[^/\s]+\/[^/\s]+$/, 'repository must be owner/name');
  assert.match(releaseSha, /^[a-f0-9]{40}$/, 'release SHA must be exact');
  assert.match(controlPlaneSha, /^[a-f0-9]{40}$/, 'control-plane SHA must be exact');
...
export function prepareReleaseCandidate({
  ...
  assertExactIdentity({ repository, releaseSha, controlPlaneSha, runId, runAttempt });
  ...
  const candidateId = `${releaseSha}:${runIdentity}`;
  const immutablePath = `/deployments/${releaseSha}/${runIdentity}.json`;
...
export function verifyReleaseCandidate({
  ...
  assert.equal(pointer.schemaVersion, 3, 'release current pointer schema drifted');
  assert.match(String(pointer.releaseSha || ''), /^[a-f0-9]{40}$/, 'release pointer release SHA is invalid');
  assert.match(String(pointer.controlPlaneSha || ''), /^[a-f0-9]{40}$/, 'release pointer control-plane SHA is invalid');
  ...
  assert.equal(pointer.immutablePath, `/deployments/${pointer.releaseSha}/${pointer.workflow?.runId}-${pointer.workflow?.runAttempt}.json`, 'release pointer path is not release-SHA addressed');
  assert.equal(pointer.artifact.candidateId, `${pointer.releaseSha}:${pointer.workflow?.runId}-${pointer.workflow?.runAttempt}`, 'release candidate ID is not release-SHA addressed');
  assert.equal(pointer.workflow?.controlPlaneSha, pointer.controlPlaneSha, 'release pointer workflow/control-plane SHA mismatch');
  ...
  assert.equal(manifest.schemaVersion, 4, 'release manifest schema drifted');
  assert.equal(manifest.repository, pointer.repository, 'release repository pointer/manifest mismatch');
  assert.equal(manifest.releaseSha, pointer.releaseSha, 'release SHA pointer/manifest mismatch');
  assert.equal(manifest.controlPlaneSha, pointer.controlPlaneSha, 'control-plane SHA pointer/manifest mismatch');
  ...
  assert.equal(manifest.workflow?.controlPlaneSha, manifest.controlPlaneSha, 'release manifest workflow/control-plane SHA mismatch');
  ...
  if (expectedReleaseSha) assert.equal(manifest.releaseSha, expectedReleaseSha, 'release SHA mismatch');
  if (expectedControlPlaneSha) assert.equal(manifest.controlPlaneSha, expectedControlPlaneSha, 'control-plane SHA mismatch');

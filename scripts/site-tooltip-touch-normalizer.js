...
let next = normalize(source, START_V1, START_V2, 'Site tooltip touch sequence reset');
if (count(next, TOUCH_V3) === 1) {
  if (count(next, TOUCH_V1) !== 0 || count(next, TOUCH_V2) !== 0) {
    throw new Error('Site tooltip touchend contract has mixed migration stages');
  }
} else {
  if (count(next, TOUCH_V2) === 0) next = normalize(next, TOUCH_V1, TOUCH_V2, 'Site tooltip touchend timestamp contract');
  next = normalize(next, TOUCH_V2, TOUCH_V3, 'Site tooltip anchor-bound click token');
}
if (count(next, CLICK_V4) === 1) {
  if (count(next, CLICK_V1) !== 0 || count(next, CLICK_V2) !== 0 || count(next, CLICK_V3) !== 0) {
  ...
  next = normalize(next, CLICK_V3, CLICK_V4, 'Site tooltip one-shot compatibility click suppression');
}
...
console.log('✅ Site tooltip touch contract normalized with anchor-bound WebKit click dedupe and desktop portal hit-testing');

...
let next = source;
if (count(next, START_V3) === 1) {
  if (count(next, START_V1) !== 0 || count(next, START_V2) !== 0) throw new Error('Site tooltip touchstart contract has mixed migration stages');
...
if (count(next, CLICK_V5) === 1) {
  if (count(next, CLICK_V1) !== 0 || count(next, CLICK_V2) !== 0 || count(next, CLICK_V3) !== 0 || count(next, CLICK_V4) !== 0) {
...
console.log('✅ Site tooltip touch contract normalized with event-bound WebKit click dedupe, keyboard preservation, desktop portal hit-testing and a 28px geometric pointer corridor');

...
REPORT = Path('reports/reader-r4-public-surface-inventory.json')
report = json.loads(REPORT.read_text(encoding='utf-8'))

for row in report['rows']:
    profile_path = Path(row['profileFile'])
    profile = json.loads(profile_path.read_text(encoding='utf-8'))
    rebuilt = {}
    inserted = False
    for key, value in profile.items():
        if key in {'surfaceContractVersion', 'surface', 'seriesShape'}:
            continue
        rebuilt[key] = value
        if key == 'routeType':
            rebuilt['surfaceContractVersion'] = 1
            rebuilt['surface'] = row['surfaceCandidate']
            if row['surfaceCandidate'] == 'series':
                rebuilt['seriesShape'] = row['shapeCandidate']
            inserted = True
    if not inserted:
        rebuilt['surfaceContractVersion'] = 1
        rebuilt['surface'] = row['surfaceCandidate']
        if row['surfaceCandidate'] == 'series':
            rebuilt['seriesShape'] = row['shapeCandidate']
    profile_path.write_text(json.dumps(rebuilt, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')

Path('scripts/lib/public-surface-registry.js').write_text(r"""'use strict';
...
Path('scripts/public-surface-registry-audit.js').write_text(r"""#!/usr/bin/env node
'use strict';
...
Path('scripts/public-surface-registry-regression-test.js').write_text(r"""#!/usr/bin/env node
'use strict';
...
scripts['surface:registry:check'] = 'node scripts/public-surface-registry-audit.js'
scripts['surface:registry:test'] = 'node scripts/public-surface-registry-regression-test.js'
for key in ['migration:metadata:check', 'migration:metadata:check:strict']:
    if 'surface:registry:check' not in scripts[key]:
        scripts[key] += ' && npm run surface:registry:check'
package_path.write_text(json.dumps(package, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')

inventory_doc = Path('reports/reader-r4-public-surface-inventory.md').read_text(encoding='utf-8')
Path('docs/READER-R4-PUBLIC-SURFACE-INVENTORY-2026-07-21.md').write_text(inventory_doc, encoding='utf-8')
Path('docs/READER-R4-PUBLIC-SURFACE-REGISTRY-2026-07-21.md').write_text("""# Reader R4 — public surface registry

...
## 2026-06-22 — Refactoring 6.0 parallel pilot: `/biografii/` raw fragments retired

`/biografii/` was advanced one step further on the same independent lane: after the first split into `recent.html` / `focus.html` / era fragments / `epigraph.html` / `post-article.html`, `BiografiiMain.astro` no longer imports those raw fragments either. The landing main is now assembled from hand-authored Astro components — `BiografiiRecentSection.astro`, `BiografiiFocusSection.astro`, reusable `BiografiiEraStubSection.astro`, `BiografiiAwakeningSection.astro`, `BiografiiEpigraphSection.astro`, and `BiografiiArticleEndBlock.astro` — while preserving the premium biography DOM/classes/text/copy. `scripts/biografii-visual-parity-audit.js` was upgraded to guard this new componentized-main contract and forbid regression back to raw section imports.

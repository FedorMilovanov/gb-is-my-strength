...
|---|---|---|
| **AGENTS-r128** | 2026-06-13 | **3D-карта баптизма: Timeline landmark-mode + restrained map highlight.** Продолжение по скринам владельца: Timeline всё ещё выглядел шумным в местах скопления дат, а режим карты мог заливать сцену слишком сильным золотым пятном. В `TimelineOverlay` добавлены `visibleTimelineTicks` (показываются major/landmark ticks + ближайшие minor к выбранному году) и `hoveredTick` — hover/focus обновляет собственную тёмную карточку события вместо нативного browser-title. Заливка стран в map mode приглушена (`${color}35`→`${color}1f`, related `${color}20`→`${color}10`, opacity exact 0.9→0.66), glow-фильтр оставлен только для exact country. `konfessii:audit` усилен guards на landmark-mode и restrained map highlight. Пересборка Vite singlefile → `_app/index.html`. QA: `konfessii:audit`, `validate:all`, `audit-pro` green. |

...
| **AGENTS-r208** | 2026-06-16 | **Интерактивная карта Исхода + React/Tailwind MapApp pilot.** Заменён prelaunch-hub `/karty/ishod/` на интерактивную карту уровня `/karty/avraam/`: 11 мест с story/bible/arch/hebrew/photos/dispute/bible_extra контентом, 6 этапов тура, 4 сюжетных режима (Весь Исход / Выход из Египта / Синайский период / 40 лет странствий), переключатель сюжетов и tour controls. Маршрут от Раамсеса через море, Синай, Кадеш-Барнеа к равнинам Моава. Карта существует в двух представлениях: (1) standalone HTML с `map-engine.js` + `base-geo.svg` + fetch(`route.json`) для прямого просмотра, (2) React/Tailwind `MapApp.tsx` компонент с `@astrojs/tailwind` (preflight:false, route-scoped) для Astro-сборки. Добавлен `tailwind.config.mjs` с цветами `gb-gold/bg/txt`. Tailwind policy (§2.1) соблюдена: route-scoped exception, не глобальный курс. SVG-маркеры с цветокодировкой по этапам, waypoints археологических объектов, детальная панель с табами. `route.json` расширен: +4 фото (Wikimedia), +9 scientific_variants, +6 verified_waypoints, he_deep/bible_extra/dispute для 7 из 11 мест. Astro-маршрут переведён на legacy shadow wrapper. `astro-ishod-pilot-audit.js` обновлён под интерактивную карту. Хаб `/karty/` обновлён: Исход помечен как «Интерактивная». `maps:validate` проходит для обоих маршрутов. |

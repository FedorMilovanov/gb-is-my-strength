...
|---|---|---|
| **AGENTS-r94** | 2026-06-10 | **GBS Series World rollout (кроме Нагорной).** Внедрён универсальный мир серии для стандартных статей без новых CSS/JS-файлов: `data/series.json` расширен v2-полями (`category`, `accent*`, `timelineLabel`, `parts[].sub/cover/era/roman`); `js/series-cards.js` получил 4-й режим `[data-series-rail]` плюс мобильные/нижние/next/timeline hosts; `js/enhancements.js` получил scoped GBS scroll-spy engine с localStorage `gb-series-progress:<key>`, `gb-series-pos:<key>:<slug>`, `gbs-swipe-tip-seen`, `gbs-toc-collapsed`; `css/site.css` получил scoped `.gbs-*` стили и вариант B кинетической римской цифры. Полный GBS включён на 8 стандартных страницах: 5× Gill, `20-antisovetov-pastoru`, 2× hard-texts. Нагорная проповедь намеренно НЕ тронута (`tw.min.css`/свой мир). Старый strip-fallback сохранён минимально, классы конфликтов префиксованы `gbs-*`, метро-линия считается JS по реальным центрам кружков. Итог: `validate:all` green, `audit-pro` **151 passed · 1 warning · 0 errors**, `visual-audit` 52 pages / 156 screenshots / 0 console / 0 network / 0 unsuppressed. |

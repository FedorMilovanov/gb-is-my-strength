...
|---|---|---|
| **AGENTS-r247** | 2026-06-20 | **РЕФАКТОРИНГ 5.0 Phase 4: консолидация shadow-паттернов.** После Phase 1-3 обнаружены и исправлены критические нарушения visual parity: `/nagornaya/istochniki/`, `/nagornaya/nakhodki/`, `/nagornaya/seriya/` использовали native Astro generic `astro-card-grid` заглушки вместо legacy premium Tailwind/sidebar-вёрстки (те же регрессии, что вызвали emergency rollback в AGENTS-r244). Все три переведены на `loadLegacyFullDocument` full-document shadow. **Усилен `legacyFullDocument.ts`**: добавлено извлечение inline `<style>` блоков (критично для карт с inline CSS-бандлами), CSP `<meta http-equiv>` (защита для external tile-серверов), preload links (шрифты/route.json), theme-color meta, script blocks — теперь full-document shadow эмитит полный `<head>` без потерь. Generic `astro-card-grid` полностью устранён из production (0 страниц). Изменения: src/pages/nagornaya/istochniki/index.astro, src/pages/nagornaya/nakhodki/index.astro, src/pages/nagornaya/seriya/index.astro, src/utils/legacyFullDocument.ts, migration/page-ownership.json, AGENTS.md, docs/refactor-2026/REFACTORING_5_0_PLAN.md, docs/refactor-2026/REFACTORING_5_0_PHASE4_PLAN.md. |
...
Older changelog rows **AGENTS-r77–r126** and older 2026-06-13 map-wave rows **r131–r139** archived to `docs/AGENTS-CHANGELOG-ARCHIVE-2026-06-14.md` to keep this instruction file scannable; normative rules below remain authoritative.
...
```
karty/_engine/
├── map-engine.js          (~2276 строк) — ОСНОВНОЙ ФАЙЛ. Все карты грузят его.
...
**map-engine.js (~2276 строк) — самодостаточный:**
...
`index.html` сокращён с 4776 до 2385 строк. Движок остаётся чистым и универсальным.
...
```
karty/
├── _engine/map-engine.js     ← ДВИЖОК (~2276 строк)

...
| P11 | ⏳ | — | (a) `.gb-accuracy-title` / `.gb-accuracy-desc` color + `html.dark` варианты (4 `!important`) — конкурент `body.nagornaya-page .gb-accuracy-*` имеет ВЫШЕ специфичность (0,2,0 vs 0,1,0/0,2,0); `!important` лишний. (b) `.heading-anchor.copied` + `html.dark` вариант (2 `!important`) — конкурент `:hover` имеет 0,1,1 vs 0,2,0 у `.copied`; `!important` лишний. −6 `!important`. |

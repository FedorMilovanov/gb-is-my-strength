...
| P5 | ⏳ | — | Удалён legacy `.theme-float-btn` (AGENTS-r17 — заменён на `gb-fc-theme` в gbFloatingControls). 10 правил, ~110 строк CSS, ~1.2 КБ. Из 5 разных мест: nagornaya-page hide, touch-target list, main block, prefers-reduced-motion list, body.gb-fc-active hide-list. Заодно удалены `#themeFloat` и `#gbSearchFloat` из CSS hide-list (тоже legacy). −1 `!important`, но `−110 строк` мёртвого кода. |

...
| P6 | ⏳ | — | Удалены 4 мёртвых селектора: `.epilogue-caption`, `.epilogue-prose`, `.h-section-link:hover` (внутри no-hover группы), `.article-img.float-fallback` (base + mobile override), `.card.fx-lift` (base + :hover + dark + reduced-motion + premium polish). Все 4 проверены `grep -rn` по `.html`/`.js` — ни одного использования. −7 `!important`, −900 байт. |

...
> **Полный аудит:** AuditRepo → каталог проекта `gb-is-my-strength` → `incoming/claude-genealogy-atlas-strategy/2026-07-11/`
...
v1 = хороший прототип, НЕ ядро атласа. Сохраняем: маршрут `/rodosloviye/` (SEO),
...
**Данные `data/genealogy/v2/`**: persons / groups (кластеры атласа) / edges
...
**Движок (рекомендация, Решение №2):** `GenealogyAtlasEngine` — собственное
SVG-ядро, vanilla TS в `src/lib/genealogy-atlas/`, d3-zoom + предвычисленный layout,
...
**LOD-контракт:** L0 «Атлас» (хребет Адам→Ной→Авраам→Иаков→Давид→Христос +
...
**Статический слой (обязателен):** build-time HTML-оглавление атласа (эпохи →
кластеры → персоны, `data-pagefind-body`), noscript, print-CSS, JSON-LD Dataset.

## 3. Куда и как вводить

- URL: `/rodosloviye/` (не менять; бренд H1 «Библейский атлас родословий»). НЕ /karty/.
- Режим: strict-native-app (уже назначен в ownership/route-profile).
- Только lane: `lane/genealogy-atlas-<phase>`, Risk 2–3; PremiumControls не трогаем
...
- Гейты lane: cache-bust (при legacy-затрагивании) → validate:static-publication →
  ...
  visual-audit + interactive-audit (добавить сценарии атласа) → бюджеты §2.
- **Discoverability**: сейчас у маршрута 0 входящих ссылок (orphan). Карточку на
  главной, ссылку из karty-hub и статей включаем ТОЛЬКО по достижении atlas-grade.
...
Acceptance = 8 atlas-grade критериев (AuditRepo REPORT §S8): визуал по референсам,
...
force-layout полного графа (урок bible-family-tree: «клубок» вместо атласа).
...
- **Phase 1 «Фундамент данных»** (ветка `claude/biblical-genealogy-svg-6l6qb8`,
  ...
  - `data/genealogy/v2/` — 3056 персон (100% с ru-именем; override 29, seed 138,
    structural 13, остальное авто с review-очередью), 2053 ребра, 14 кластеров
    атласа, 76 народов, золотой хребет **Христос→Адам связан (76 узлов, все якоря)**.
